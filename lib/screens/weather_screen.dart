...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  void getDefaultCity() {
    defaultCity = Provider.of<MyLocations>(context, listen: false).getDefaultCity();
  }

  void navigateToPage(int index) {
    switch(index) {
      case 2: Navigator.of(context).pushNamed(AddLocationScreen.routeName); break;
      default: break;
    }
  }
  ...
  Widget _widgetForecast(String date, String icon, String description, String temperature) {
    return Container(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Spacer(),
              Image.network('http://openweathermap.org/img/wn/'+icon+'@2x.png', width: 24, height: 24,),
              Text(description),
              Spacer(),
              Text(temperature)
            ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: _isLoading ? Text('...') : Text(forecast[0].city + ', ' + forecast[0].country),
          elevation: 0.0,
        ),
        body: _isLoading ? Center(child: CircularProgressIndicator(),) : Column(
          children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Container(
                                ...
                                child: Image.network(
                                    'http://openweathermap.org/img/wn/'+forecast[0].icon+'@2x.png')),
                            Text(
                              forecast[0].temperature < 10 ? forecast[0].temperature.toString().substring(0,1) : forecast[0].temperature.toString().substring(0,2),
                              style: TextStyle(
                                  fontSize: 60,
                                  color: Colors.white,
                        ...
                        Container(
                            ...
                            child: Text(
                              DateFormat('dd.MM.yyyy hh:mm').format(forecast[0].date).toString(),
                              style:
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          _widgetWeatherInfoColumn(
                              ...
                              forecast[0].pressure.toString()+' hpa',
                              'Pressure'),
                          _widgetWeatherInfoColumn(
            ...
            Container(
              ...
              child: ListView.builder(itemCount: forecast.length-1 ,
              itemBuilder: (_, i) => _widgetForecast(DateFormat().add_EEEE().format(forecast[i+1].date).substring(0,3) + ' ' + DateFormat().add_d().format(forecast[i+1].date),
               forecast[i+1].icon, forecast[i+1].description, forecast[i+1].temperature < 10 ? forecast[i+1].temperature.toString().substring(0,1) + symbolUnit : forecast[i+1].temperature.toString().substring(0,2) + symbolUnit) ),
            )
          ],
        ),
        drawer: AppDrawer(),
